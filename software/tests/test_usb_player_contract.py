...
class UsbPlayerContractTest(unittest.TestCase):
    ...
    def test_ch376_byte_read_returns_single_data_packet(self):
        source = CH376_SOURCE.read_text(encoding="utf-8")
        self.assertIn("CH376_SINGLE_PACKET_READ", source)
        self.assertIn("buffer_size > CH376_SINGLE_PACKET_READ", source)
        self.assertNotIn("continue_byte_read", source)
        self.assertNotIn("BYTE_RD_GO failed", source)
