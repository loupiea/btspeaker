...
- 主控：ESP32-D0WD-V3 裸芯片。
- 外部 Flash：16MB SPI Flash，接 ESP32 专用 Flash 总线。
- 不加 PSRAM。
...
3. ESP32 最小系统、Flash、晶振、下载电路、启动绑带脚。
...
- 3.3V 给 ESP32、Flash、ES8388 数字侧、OLED、CH376S 逻辑侧、TF/SD。
- 3.3V 预留测试点。
- ESP32、ES8388、CH376S、OLED、TF/SD 每个芯片/接口旁边都要有就近 0.1uF 去耦。
...
- ESP32-D0WD-V3 供电引脚全部接 3.3V，并逐个放去耦电容。
- 40MHz 晶振靠近 ESP32，负载电容按晶振规格选择。
- 外部 16MB SPI Flash 使用 GPIO6-GPIO11 专用 Flash 总线。
- Flash 走线短、直、少过孔，不和 TF/SD、CH376S 共用。
- Flash 供电 3.3V 旁边放 0.1uF 和合适的储能电容。
- Flash 型号建议 W25Q128JV 或同级 128Mbit SPI Flash。
...
- 喇叭线和功放输出走线要远离 ESP32 天线、ES8388 模拟输入、晶振和 Flash。
...
- 底层是否能尽量保持连续 GND。
- ESP32 天线区域是否有明确净空。
- ESP32、Flash、晶振是否能靠近摆放。
- IP5306、SY8088、电感、大电流电容是否能形成短回路。
- MAX98357A 是否靠近喇叭接口。
- 喇叭输出是否远离 AUX、晶振、Flash、天线和 USB 数据线。
